...
def draw_polygons( matrix, screen, color ):        
   ...
   #  print "drawing"
     ...
     while point < len(matrix) - 2:
         ...
         P = [matrix[point+1][0] - matrix[point][0],matrix[point+1][1] - matrix[point][1],matrix[point+1][2] - matrix[point][2]]
         Q = [matrix[point+2][0] - matrix[point][0],matrix[point+2][1] - matrix[point][1],matrix[point+2][2] - matrix[point][2]]

         norm = P[0]*Q[1] - P[1]*Q[0]
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+1):
            ...
            add_edge(edges, points[index][0],
                     ...
                     '''
            if lat != lat_stop-1:
                add_polygon(edges,points[(index+1) % len(points)][0], points[(index+1) % len(points)][1], points[(index+1) % len(points)][2], points[(index+num_steps) % len(points)][0], points[(index+num_steps) % len(points)][1], points[(index+num_steps) % len(points)][2], points[index][0], points[index][1], points[index][2])

            if longt != longt_stop:
                add_polygon(edges, points[(index+1) % len(points)][0], points[(index+1) % len(points)][1], points[(index+1) % len(points)][2], points[(index+num_steps) % len(points)][0],points[(index+num_steps) % len(points)][1], points[(index+num_steps) % len(points)][2],points[(index+num_steps+1) % len(points)][0], points[(index+num_steps+1) % len(points)][1], points[(index+num_steps+1) % len(points)][2])
